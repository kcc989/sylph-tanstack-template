...
The Worker wraps every application request with a durable writer gate in a separate recovery-control D1 database. Keep the control database outside application restore. Application state must remain in the declared D1 databases, optional R2 buckets, the managed KV and Queue adapters, or explicitly registered Durable Objects. Declare resources in `applicationBucketBindings`, `managedKvBindings`, `managedQueueBindings`, and `managedDurableObjectBindings` in `scripts/sylph-resources.ts`. Use the Layers in `src/managed.ts`; native KV and Queue bindings are transport only. Add idempotent Queue handlers in `src/managed-queue-handlers.ts`. Registered Durable Object classes must use the object writer gate and delegate recovery to the snapshot helper with an explicit cache-reload callback. All writers run through the recovery gate. Scheduled handlers, external storage, and other background writers require a separately verified adapter before use.
